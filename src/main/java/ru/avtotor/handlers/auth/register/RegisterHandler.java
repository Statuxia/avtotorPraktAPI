...
import ru.avtotor.exceptions.InvalidDataException;
...
public class RegisterHandler extends DefaultHandler {
    ...
    public RegisterHandler(String endpoint, byte method, Params params) {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            JSONObject response = new Register(request).register();
            sendResponse(exchange, (short) response.getInt("code"), response.toString(INDENT_FACTOR).getBytes(StandardCharsets.UTF_8));
        } catch (NoDataFoundException | InvalidDataLengthException | WeakPasswordException |
    ...
    public void deepValidateData2(JSONObject request) throws InvalidDataLengthException, JSONException, WeakPasswordException, InvalidDataException {
        ...
        for (String key : columnLengths.keySet()) {
            ...
            if ("sex".equals(key)) {
                if (!("муж".equals(value) || "жен".equals(value))) {
                    throw new InvalidDataException("Некорректно указан пол. Пол может быть либо 'муж', либо 'жен'");
                }
            }
